...
def parse_args():
    ...
    parser.add_argument('-P', '--profiling',
                        choices=['perf', 'gprof'],
                        help='Enable profiling using Linux Perf or gprof')
...
def get_default_vpc(session, region, az):
    ...
    res = ec2.describe_vpcs(Filters=[{'Name': 'isDefault',
                                      'Values': ['true']}])
...
def build(
        session, region, ami, subnet, repository, commit_id, bucket_name,
        perf_test_id, build_file, profiling):
    ...
    stack = cfn.create_stack(
            ...
            Parameters=[
                ...
                },
                {
                    ...
                    'ParameterValue': str(profiling and profiling == 'perf')
                },
                {
                    'ParameterKey': 'UseGProf',
                    'ParameterValue': str(profiling and profiling == 'gprof')
                },
            ],
    ...
    instance_ids = [stack.outputs[0]['OutputValue']]
    if profiling:
        instance_ids.append(stack.outputs[1]['OutputValue'])

    logger.info(region + ': Waiting for release build on ' + instance_ids[0])
    if profiling:
    ...
    try:
        ...
        s3.head_object(Bucket=bucket_name,
                       Key=perf_test_id + '/release/goto-cc')
    except:
    ...
    if profiling:
        try:
            s3.head_object(Bucket=bucket_name,
                           Key=perf_test_id + '/profiling/cbmc')
            s3.head_object(Bucket=bucket_name,
                           Key=perf_test_id + '/profiling/goto-cc')
        except:
...
def prepare_sqs(session, region, perf_test_id, tasks, profiling):
    # create a bucket for storing artifacts
    logger = logging.getLogger('perf_test')
    ...
    seed_queue(session, region, queue, tasks, profiling)

    return (queue, arn, url)


def seed_queue(session, region, queue, task_set, profiling):
    # set up the tasks
    logger = logging.getLogger('perf_test')
    ...
    configs = ['release']
    if profiling:
        configs.append('profiling')
    for c in configs:
        for t in tasks:
            response = queue.send_messages(
                    Entries=[{'Id': '1', 'MessageBody': c + '-' + t}])
...
def main():
    ...
    perf_test_id = re.sub(r'[:/_\.\^~ ]', '-', perf_test_id)
